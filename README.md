...
`memwarden down --all` reverses everything it wrote; `--data` deletes the brain. Prefer to try before you install? `npx memwarden audit <store>` needs no daemon and no setup. macOS launchd and POSIX detached daemon logs are owner-only (`0600`) and checked every minute: each oversized current log is reduced in place while at most one 1 MiB tail is retained as `daemon.log.1`; systemd continues to use journald's own rotation. Windows retains its v0.1.0 detached logging lifecycle, so these POSIX mode and launchd-style rotation guarantees do not apply there.
