...
describe("mem::consolidate-pipeline", () => {
  ...
  it("keeps a legacy short per-file Memory beside the new claim-specific id", async () => {
    ...
    const r = await consolidate();
    expect(r.folded).toBe(3);
    // Import migration may add the split filesystem identity, but claim
    // consolidation must not overwrite any legacy content or lineage.
    expect(await kv.get<Memory>(KV.memories, legacyId)).toEqual({
      ...legacy,
      projectPath: "proj-a",
    });
    const memories = await kv.list<Memory>(KV.memories);
