//
// CJK-aware segmentation for the BM25 tokenizer. Ported from the original
// src/state/cjk-segmenter.ts. The optional native segmenters
// (@node-rs/jieba for Chinese, tiny-segmenter for Japanese) are loaded via
// createRequire and degrade to whole-string tokenization when absent, so
// this carries over with NO new dependencies: if the optional packages are
// not installed the fallback path runs and search still works for CJK input
// (just coarser). Korean uses a pure regex block splitter (no dep).
