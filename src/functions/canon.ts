...
export function memoryMatchesCanonProject(
  ...
  const memoryIdentity = resolveMemoryIdentity(memory);
  return (
    memoryIdentity.projectKey === identity.key ||
    projectIdentityMatchesPath(memoryIdentity, identity.root)
  );
}
...
export type CanonImportResult =
  | {
      ...
      id: string;
      projectKey: string;
      /** Local raw-hash verdict, independent of semantic lifecycle/attestation. */
      verdict: "verified";
