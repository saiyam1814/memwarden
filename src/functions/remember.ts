...
export async function rememberMemory(
  ...
  const project = canonicalizePath(resolve(input.project.trim()));
  let files: string[];
  ...
  try {
    ...
    files = normalizeManualFiles(project, input.files ?? []);
  ...
  const id = manualMemoryId(project, kind, title, content);
  if (supersedes === id) {
    return { success: false, reason: "a memory cannot supersede itself" };
  }

  return withKeyedLock(`remember:${project}`, async () => {
    ...
    if (
      previous?.project &&
      canonicalizePath(previous.project) !== canonicalizePath(project)
    ) {
      return {
        success: false,
    ...
    if (files.length > 0) {
      ...
      const hashes = hashFiles(files, project);
